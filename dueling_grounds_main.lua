...
local Window = Library:CreateWindow({
    Title = "Dueling Grounds Auto Parry",
    SubTitle = "Discovery / No Learning",
...
--==================================================
-- DUELING GROUNDS DEPENDENCY / STATE BRIDGE
--==================================================
...
local AutoTargetNearest    = NewValueControl(true)
...
local ParryingAnimation = {} -- Fill later if discovered
local ParryFailed = {} -- Fill later if discovered

local AutoParryRange = 18
local MaxCycleRange = 45
local ParryWindow = 0.2
local ProbabilityToParry = 100
local DefaultReactionTime = 0.15
local ParryOffset = 0
local BlockHoldTime = 0.22
...
local function GetAllCharactersInFolder()
    ...
    -- Prefer a selected workspace folder when the game exposes one.
    if SelectedFolder and game.Workspace:FindFirstChild(SelectedFolder) then
        ...
        for _, Character in SelectedFolderObject:GetChildren() do
            if Character.ClassName == "Model"
                and Character:FindFirstChildWhichIsA("Humanoid") then

                if not IncludeLocalCharacter
    ...
    -- Dueling Grounds fallback: use actual Player.Character models directly.
    ...
    if ok and type(players) == "table" then
        for _, player in ipairs(players) do
            ...
            if character
                ...
                and (IncludeLocalCharacter or player ~= LocalPlayer) then
...
--==================================================
-- DUELING GROUNDS AUTOPLAY RUNTIME (restored from original)
--==================================================
local Receptors = {
    ["Receptor1"] = "X",
    ["Receptor2"] = "C",
...
local function LogAnimation(assetId, trackInfo)
    if not AnimationsLoggedCache[assetId] then
        ...
        print(string.format(
            "[DG ANIMATION] NEW | ID=%s | Name=%s",
            tostring(assetId),
            tostring(trackInfo.Name or "Unknown")
        ))
    end
end
...
local function EvaluateAnimation(anim, character, localCharacter, localRoot, targetRoot, currentActiveIds)
    ...
    local assetId = tostring(anim.AnimationId)
    local attackConfig = GameConfig[assetId]
...
local function BuildTimingControls()
    ...
    for _, style in ipairs(styleNames) do
        ...
        for _, entry in ipairs(animations) do
            ...
            -- matching the behavior of the original Dueling Grounds UI.
...
--==================================================
-- WABI CONTROLS FOR DUELING GROUNDS RUNTIME VALUES
--==================================================
...
Library:Notify({
    Title = "Loaded",
    Content = "Dueling Grounds ESP + animation logger loaded. No learning/database.",
    Duration = 4
})
...
print("Dueling Grounds Auto Parry | Wabi discovery build loaded")
